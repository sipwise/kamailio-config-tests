...
parallel_execution() {
  ...
  for t in ${SCENARIOS}; do
    ...
    # TODO: logs should be saved somewhere and then moved in the proper directory.....
    #"${BIN_DIR}/check.sh" "${OPTS[@]}" -d ${DOMAIN} -p "${PROFILE}" -s "${GROUP}" "$t" > "${log_temp}/execution.log" 2>&1 &
    "${BIN_DIR}/check.sh" -L "${OPTS[@]}" -d "${DOMAIN}" -p "${PROFILE}" -s "${GROUP}" "$t" > "/tmp/${t}_execution.log" 2>&1 &
  ...
  # TODO: here I have to mode all the logs in the correct folder....
  mv /tmp/*_execution.log "${LOG_DIR}"
