...
grep_logs() {
  #Creating the used call-id
  call_id=NGCP%$(basename "${SCEN_CHECK_DIR}")%
  # grep the kamailio log
  grep "${call_id}" "${KAM_LOG}" > "${LOG_DIR}/kamailio.log"
  if [ -f "${SEMS_LOG}" ] ; then
    # grep the sems log
    grep "${call_id}" "${SEMS_LOG}" > "${LOG_DIR}/sems.log"
  fi
  if [ -f "${SEMS_PBX_LOG}" ] ; then
    # grep the sems-pbx log
    grep "${call_id}" "${SEMS_PBX_LOG}" > "${LOG_DIR}/sems-pbx.log"
  fi
  # grep the kamailio-lb log
  grep "${call_id}" "${KAMLB_LOG}" > "${LOG_DIR}/kamailio-lb.log"
}
