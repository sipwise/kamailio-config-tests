...
function check_test
{
  ...
  if [ -n "${FIX_RETRANS}" ] ; then
    echo -n "$(date) - Fix retransmissions enabled: try to test the next json file"
    local next_msg
    local next_tap
    next_test_filepath "$1"
    next_tap=$(echo "$3"|sed 's/_test\.tap/_test_retry\.tap/')

    echo -n "$(date) - Testing $(basename "$1") againts $(basename "$next_msg") -> $(basename "$next_tap")"
  ...
  if [ -z "${GRAPH}" ] && [ -n "${GRAPH_FAIL}" ]; then
    ...
    # In any ifailure case only the graph related to the original json file will be created 
    graph "$msg" "${dest}.png"
    echo "$(date) - Done"
  fi
}
...
function next_test_filepath
{
  ...
  old_json=$(echo "${old_json}" | sed 's/^0*//')
  new_json=$((${old_json}+1))
  ...
  if [ -n "${FIX_RETRANS}" ] ; then
    ...
    for json_file in "${file_find[@]}" ; do
      ...
      for next_json_file in "${file_find[@]}" ; do
        if ( diff -q -u <(tail -n3 "${json_file}") <(tail -n3 "${next_json_file}") &> /dev/null ) ; then
          echo "$(basename "${next_json_file}") ---->  Retransmission: renaming the file in ${next_json_file}_retransmission"
    ...
    if RETRANS_ISSUE ; then
      ...
      for json_file in ${file_find[@]} ; do
        ...
        mv -n "${json_file}" "/usr/share/kamailio-config-tests/log/scenarios/invite_tcp_udp_reinvite/${new_name}" &> /dev/null
        let a=a+1
      done
    fi
  fi
