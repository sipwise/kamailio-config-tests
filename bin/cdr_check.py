...
def check_single_cdr_recursive(scen, msg):
    ...
    for rule, value in scen.iteritems():
        ...
        if value.startswith('_:NOT:_'):
            ...
            msg_ok = '%s match'
            msg_ko = '%s not match'
        print 'value: %s - rule: %s' % (value, msg[rule])
        result = re.search(value, msg[rule])
        if (result is not None) == flag:
...
def check_cdr_recursive(scen, msgs, test):
    ...
    for i in (range(num_scen)):
        ...
        for message in msgs:
            ...
            validated, comments, oks = check_single_cdr_recursive(scen[i], message)
...
def usage():
    print 'Usage: mysql_check.py [-h] [-d] [-y] [-j] [-t] cdr_file cdr_test.yml'
    print '-h: this help'
    print '-d: debug'
